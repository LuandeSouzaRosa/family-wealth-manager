...
async def run_test():
    ...
    try:
        ...
        # -> Fill the transaction description with 'Transferência para reserva', fill amount with '100', then open the Type dropdown to select 'Transferência' (open combobox).
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[3]/div[3]/form/div[2]/input').nth(0)
        await asyncio.sleep(3); await elem.fill('Transferência para reserva')
        ...
        assert await frame.locator("xpath=//*[contains(., 'Salvando')]").nth(0).is_visible(), "Expected 'Salvando' to be visible"
        assert await frame.locator("xpath=//*[contains(., 'Transferência para reserva')]").nth(0).is_visible(), "Expected 'Transferência para reserva' to be visible"
