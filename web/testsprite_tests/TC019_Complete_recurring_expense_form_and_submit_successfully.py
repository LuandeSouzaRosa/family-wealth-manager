...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Recorrentes' navigation item (labeled 'Custos Fixos' on the navbar) to open the recurring/ fixed costs page.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/header/div/nav/a[3]').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Click the 'Nova Recorrência' button (index 934) to open the add recurring-expense dialog so fields can be filled.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/main/div/div/div/div[3]/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Fill the Descrição field with 'Internet Teste', enter a valid Valor (e.g., 1500), then click 'Salvar' to create the recurring expense.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[3]/div[3]/form/div[2]/input').nth(0)
        await asyncio.sleep(3); await elem.fill('Internet Teste')
        ...
        # -> Open the Categoria combobox to choose a category (click element index 1072) so the missing required field can be filled, then re-submit the form.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[3]/div[3]/form/div[3]/div[2]/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Select the 'Moradia' category (option index 1182) and then click the 'Salvar' button (index 1076) to submit the form and create the recurring expense.
        frame = context.pages[-1]
        # Click element
