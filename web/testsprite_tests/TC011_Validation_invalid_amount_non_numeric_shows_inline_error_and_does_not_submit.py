...
async def run_test():
    ...
    try:
        ...
        # -> Click the '+ Nova Transação' button to open the New Transaction modal so the amount field can be tested for invalid input.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/main/div/div/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Fill the transaction description, enter invalid amount 'abc' into the Valor field, click 'Salvar Transação' to attempt submit (expect validation to block and modal to remain open). Then verify modal remains open and amount input rejected.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[3]/div[3]/form/div[2]/input').nth(0)
        await asyncio.sleep(3); await elem.fill('Teste valor inválido')
