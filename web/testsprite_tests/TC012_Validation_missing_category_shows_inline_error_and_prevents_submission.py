...
async def run_test():
    ...
    try:
        ...
        # -> Fill email and password, then click the 'Entrar' button to submit the login form.
        ...
        # -> Click the '+ Nova Transação' button to open the new-transaction form (element index 832).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/main/div/div/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Fill 'Sem categoria' into the description field, fill '10' into the amount field, click 'Salvar Transação', wait briefly, then check the page for the inline validation text containing 'categoria'.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[3]/div[3]/form/div[2]/input').nth(0)
        await asyncio.sleep(3); await elem.fill('Sem categoria')
