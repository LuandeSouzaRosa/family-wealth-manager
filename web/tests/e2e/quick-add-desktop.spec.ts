...
test.describe('Quick Add Desktop E2E', () => {
  test('deve garantir submit unico e persistencia unica no Quick Add (desktop)', async ({ page }) => {
    ...
    // Quick Add atual e mobile-only (md:hidden). Mantemos projeto desktop, com viewport
    // estreito, para validar o mesmo fluxo sem alterar produto.
    await page.setViewportSize({ width: 390, height: 844 });

    const token = buildLettersOnlyToken();
    const runId = buildUniqueDescription('E2E_QA_DESKTOP').split('_').at(-1) ?? 'run';
    ...
    try {
      ...
      console.log(
        `QuickAdd Desktop E2E: run=${runId} input="${rawInput}" expected="${expectedDescription}" submitCount=${quickAddSubmitRequestCount}`
      );
    } finally {
      await cleanupTransactionsByDescription(supabaseClient, expectedDescription);
